# JG-ORM — Customer & Data Management
...
- **고객 관리**: 다수 고객 DB, 자동 검색(이름·주소·전화 substring·주민번호 앞/뒤·통화결과·담당자), 페이지네이션, 통화결과 컬러 배지
...
- **관리자**: 사용자 CRUD + 비밀번호 재설정, 담당자별 권한 2종 체크박스, 담당자 일괄 변경, 변경 이력 뷰어, 고객 삭제(권한 부여 시 담당자도 가능)
- **모바일**: 햄버거 메뉴, 테이블 가로 스크롤, 팝업 풀스크린, 검색바 2열 그리드
- **브랜드**: JG-ORM 청록 `#0891b2`, 딥블루 `#1e3a8a`, Pretendard 폰트
...
```bash
git clone https://github.com/Cayson-Choi/a1-insurance jg-orm
cd jg-orm
pnpm install
```
...
Vercel 프로젝트 → **Storage** 탭 → **Add Integration** → **Neon** → 기존 프로젝트 연결
...
public/brand/jg-orm-logo.png     JG-ORM 로고
```
...
- **엑셀 대량 이관**: 관리자 > 엑셀 업/다운로드 → 미리보기 → 업로드 실행
- **담당자 일괄 재배정**: 고객 목록에서 체크박스 다중 선택 → 플로팅 바
- **주민번호 열람**: 고객 상세 팝업 → "복호화" 버튼 (변경 이력에 자동 기록)
- **변경 이력 감사**: 관리자 > 변경 이력 (액션·작업자·기간별 필터)
- **개인정보 유출 대응**: 주민번호는 AES-256-GCM 암호화 + HMAC 해시 이중 저장. 평문은 DB에 없음.
