...
- **인증 / 권한**: Auth.js v5 Credentials, JWT 8h 세션, 유휴 30분 자동 로그아웃. 3단계 역할 — **관리자(admin)** · **매니저(manager, 전체 조회 + 담당자 변경 기본)** · **담당자(agent)** — 에 권한 5종(`canCreate`·`canEdit`·`canDelete`·`canExport`·`canDownloadImage`) 개별 부여
...
- **일괄 편집**: **담당자 일괄 변경** (admin + manager) · **DB 등록일 일괄 변경** (admin 전용) — 각 동작마다 변경 이력 자동 기록
- **엑셀 I/O**: 28컬럼 import/export, 평문 주민번호 처리. **업로드 = Upsert** — 고객코드 → 주민번호(앞+뒤 13자리) → 이름+전화 순 매칭으로 업데이트, 매칭 실패 시만 신규 추가. 엑셀에 없는 기존 고객은 유지, 빈 셀은 기존 값 보존.
...
- **관리자**: 사용자 CRUD + 비밀번호 재설정, 권한 5종 체크박스(입력·수정·삭제·엑셀·이미지), 담당자 일괄 변경, DB 등록일 일괄 변경, 변경 이력·로그인 이력 뷰어
...
`role` (`admin` / `manager` / `agent`) + 담당자 권한 5종의 조합으로 구성됩니다.
...
|---|---|---|
| `admin` | 전부 | 없음 |
| `manager` | 전체 고객 조회 · 담당자 재할당(개별+일괄) | 관리자 전용 페이지 · DB 등록일 일괄 변경 |
| `agent` | 본인 배정 고객 조회/팝업 + 방문주소·메모·통화결과·예약일시 편집 | 타인 고객 · 담당자 변경 · 관리자 페이지 |

### 권한 플래그 5종 (agent·manager 공통, admin 은 자동으로 전부 보유)

| 플래그 | 영향 범위 |
|---|---|
| `canCreate` | 엑셀 업로드(신규 고객 등록) — `/admin/excel` 페이지 접근 + import API |
| `canEdit` | 고객 상세 모든 필드 수정 (없으면 방문주소·메모·통화결과·예약일시만 가능) |
| `canDelete` | 고객 삭제 버튼 |
| `canExport` | 엑셀 다운로드 |
| `canDownloadImage` | 고객 팝업의 **이미지 저장** 버튼 노출 |

- `admin` 은 위 5종을 자동으로 모두 보유합니다. `manager` 는 agent 와 동일하게 관리자가 필요한 플래그만 켭니다.
- **담당자 일괄 재배정**: admin 또는 manager. **DB 등록일 일괄 변경**: admin 전용.
- **사용자 관리 · 변경 이력 · 로그인 이력 뷰어 · 엑셀 업/다운로드 페이지**: admin 전용.
- `manager` 도 `customer.agent_id` 로 할당 가능(영업 겸직). 관리자는 다른 사용자를 **강제 로그아웃** 가능 (본인 제외). 대상 사용자는 다음 페이지 이동 시 자동으로 `/login` 으로 돌아갑니다.
