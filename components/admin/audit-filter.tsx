...
export function AuditFilterBar({
  ...
  const [action, setAction] = useState<string>(sp.get("action") ?? "");
  const [actor, setActor] = useState<string>(sp.get("actor") ?? "");
  ...
  const hasFilter = !!action || !!actor || !!from || !!to;
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1.5 w-[calc(50%-0.25rem)] md:w-36">
        ...
        <Select
          value={action || ALL}
          onValueChange={(v) => setAction(!v || v === ALL ? "" : String(v))}
        >
          <SelectTrigger className="h-9 w-full">
            <span>{action ? ACTION_LABEL[action] : "전체"}</span>
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="flex flex-col gap-1.5 w-[calc(50%-0.25rem)] md:w-44">
        ...
        <Select
          value={actor || ALL}
          onValueChange={(v) => setActor(!v || v === ALL ? "" : String(v))}
        >
          <SelectTrigger className="h-9 w-full">
            <span className="truncate">
              {actor
                ? `${actors.find((a) => a.agentId === actor)?.name ?? actor} · ${actor}`
                : "전체"}
            </span>
          </SelectTrigger>
          <SelectContent>
