...
| 작성일 | 2026-04-18 |
| 버전 | 1.0 (초기 납품 준비 완료) |
| 상태 | **모든 기능 구현 완료** · Production build 통과 · 납품 문서 준비 완료 |
...
| 역할 | 인원 (예상) | 권한 |
|---|---|---|
| **관리자 (admin)** | 1~2명 | 모든 기능. 사용자 CRUD, 담당자 일괄 재배정, 엑셀 업/다운, 감사로그 열람, 주민번호 복호화 열람 |
| **담당자 (agent)** | 2~10명 | 본인 배정 고객만 목록·상세·편집·팝업 이미지 저장. 엑셀/담당자 변경 불가 |
...
- 이름, 주소(부분 일치), 주민번호 앞 6자리, 주민번호 뒤 6자리, 통화결과 — 복합 AND 조건
- 주민번호 검색은 HMAC 해시 비교 (DB엔 원문 없음)

### F3. 팝업 상세 편집 (Intercepting Route 모달)

- 목록에서 행 클릭 → 팝업 오버레이 (페이지 새로고침 없음)
- 편집 필드: 성명 / 주민번호 / 연락처 / 원주소 / 방문주소 / 직업 / 통화결과 / 보험사 / 보험상품명 / 가입일 / 예약일자 / 메모
- 통화결과 enum: **예약 · 부재 · 가망 · 거절 · 결번 · 민원**
- 이전/다음 버튼으로 검색 결과 내 고객 간 이동 (검색 컨텍스트 유지)
- 키보드 단축키: `Esc` 닫기 · `←/→` 이전/다음 · `Ctrl+S` 저장
- 저장 시 감사로그 기록 (before/after JSON)

### F4. 담당자 변경 + 권한 제어

- **관리자만**: 고객 상세에서 담당자 드롭다운 변경 (건별)
- **관리자만**: 목록 체크박스로 다중 선택 후 "담당자 일괄 변경"
- 일괄 변경 시 확인 다이얼로그 + "되돌릴 수 없습니다" 경고 + audit log 기록
- **담당자**: 본인 배정 고객만 보임 (쿼리 레벨에서 차단, URL 직접 접근도 403)

### F5. 엑셀 업/다운로드

- **관리자만** 업/다운
- 업로드: `고객명부.xlsx` 28컬럼 포맷 호환. 프리뷰 → 검증(zod) → upsert(`customer_code` 기준)
- 다운로드: 현재 적용된 검색 쿼리 결과를 엑셀 스트림으로 응답
- 한글 컬럼명·UTF-8 인코딩 보장

### F6. 팝업 이미지 저장

- 팝업 상세 화면을 **보이는 그대로 PNG로 저장** (영업 기록·보고용)
- 파일명 `{고객명}_{YYYYMMDD-HHmm}.png`
- 편집 중인 입력값도 DOM에 반영된 상태로 캡처
- 한글 폰트(Pretendard) 임베드 — 깨짐 없음

### F7. 관리자 페이지

- 사용자(담당자) CRUD — 추가·수정·비밀번호 리셋
- 감사로그 뷰어 — 필터(기간·action·actor·customer), 페이지네이션
- 주민번호 복호화 열람 시 감사로그 자동 기록
