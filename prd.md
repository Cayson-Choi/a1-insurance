...
담당자는 기본적으로 **읽기 전용 + 방문주소·메모·통화결과만 편집 가능**이며, 관리자가 아래 5개 권한을 개별 부여한다:
...
`id, agent_id UNIQUE, password_hash, name, role('admin' | 'agent'), can_create, can_edit, can_delete, can_export, can_download_image, created_at, last_login_at, sessions_invalidated_at, last_seen_at`
