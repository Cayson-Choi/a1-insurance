# JK-CRM — 제품 요구사항 문서 (PRD)

| 항목 | 값 |
|---|---|
| 제품명 | **JK-CRM** (Customer & Data Management) |
...
JK-CRM은 보험·금융 영업 담당자용 **고객 및 데이터 관리 도구**다. 엑셀로 분산 관리되던 고객 정보를 웹으로 이관하여 다음을 제공:

1. 엑셀 28컬럼 데이터의 무손실 import / export 호환
2. 담당자별 할당·세분화된 권한 제어
3. 변경 이력(감사로그) 자동 기록 및 관리자 열람
...
| 역할 | 인원 (예상) | 권한 |
|---|---|---|
| **관리자 (admin)** | 1~2명 | 모든 기능 자동 부여. 사용자 CRUD · 담당자 일괄 재배정 · 엑셀 업/다운 · 감사로그 열람 · 고객 삭제 · 담당자 변경 |
...
담당자는 기본적으로 **읽기 전용 + 방문주소·메모·통화결과만 편집 가능**이며, 관리자가 아래 4개 권한을 개별 부여한다:
...
관리자(admin)는 위 4개 권한을 암묵적으로 모두 포함하며, 추가로 **담당자 변경 · 일괄 재배정 · 사용자 관리 · 변경 이력 뷰어 · 전체 고객 접근**이 가능.
...
- 리스트 컬럼: 담당자(관리자만) / 이름 / 연락처 / 생년월일 / 주소 / 직업 / 통화결과 / 보험사 / DB만기일
...
- 관리자 또는 `canDelete` 있는 담당자: 팝업 우상단 "삭제" 버튼 → 확인 다이얼로그 → 삭제 전 감사로그 기록 후 영구 삭제
...
- 사용자 추가 시 권한 체크박스 4종: `입력` · `수정` · `삭제` · `엑셀 다운로드`
...
- [x] 모든 화면에 JK-CRM 로고 + "JK-CRM 고객·데이터 관리" 헤더, 풋터
- [x] 로그인 화면: JK-CRM 로고 + 태그라인 "Customer & Data Management"
...
- [x] favicon · title · meta 태그에 JK-CRM 브랜드
...
- 파일: `public/brand/jk-crm-logo.png`
- 디자인: 청록·파랑 그라디언트의 동그라미 안에 인물·식물·네트워크 아이콘 + "JK-CRM" 텍스트 + "CUSTOMER & DATA MANAGEMENT" 부제
...
`id, agent_id UNIQUE, password_hash, name, role('admin' | 'agent'), can_create, can_edit, can_delete, can_export, created_at, last_login_at`
...
```
app/
  (auth)/login/page.tsx                        로그인 (JK-CRM 로고 + 태그라인)
...
lib/
  ...
  auth/rbac.ts                                 권한 헬퍼 (canCreate/canEdit/canDelete/canExport)
  customers/actions.ts                         Server Actions (update/delete/bulkReassign)
...
drizzle/                                       마이그레이션 SQL
public/brand/jk-crm-logo.png                   브랜드 로고
```
...
| 12 | **JK-CRM 브랜드 리뉴얼** (청록 컬러 팔레트 · 새 로고) | ✅ |
...
- `pnpm db:migrate` → 스키마 반영 (마이그 0000–0005)
...
4. 환경변수 `AUTH_SECRET` 등록 (Production · Preview)
...
| 2026-04-19 | 1.2 | **브랜드 확정: JK-CRM (Customer & Data Management)**. 청록 컬러 팔레트(`#0891b2`) 적용, 로고(`public/brand/jk-crm-logo.png`), 풋터/로그인/메타데이터/엑셀 파일명/감사로그 라벨 통일, 운영 가이드 일반화 | 개발팀 |
