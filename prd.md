...
- 업로드: 28컬럼 포맷 호환. 프리뷰(앞 10행·오류·미등록 담당자·업데이트·신규·기존 유지 건수) → 검증(zod) → **upsert**
  - 매칭 우선순위: `customer_code` → `rrn_front+rrn_back` (앞+뒤 13자리 조합) → `name + phone1(숫자만)` → 매칭 없으면 신규 INSERT
  - **엑셀 빈 셀은 기존 값 보존** (덮어쓰지 않음). 웹 전용 필드(`memo`, `reservationAt`)는 엑셀에 없으므로 항상 보존.
  - **엑셀에 없는 기존 고객은 유지** (자동 삭제 없음). 삭제는 팝업 개별 삭제 버튼으로만 가능.
  - 전체 트랜잭션으로 감싸 중간 실패 시 모두 롤백.
...
auth.ts                                        Auth.js 설정 (JWT, brute-force lockout, 세션 무효화)
auth.config.ts                                 Auth.js authorized 콜백 (proxy.ts 에서 실행)
...
lib/
  ...
  security/
    pii.ts                                     PII 암호화/복호화 (AES-256-GCM + HMAC-SHA256)
    rate-limit.ts                              API rate limit + same-origin 검사 + 보안 헤더
    audit.ts                                   감사로그 민감 데이터 마스킹
    ids.ts                                     UUID 검증·정규화
...
| 2026-04-24 | 1.9.0 | **엑셀 업로드 정책 변경: 전체 교체 → Upsert**. 기존 "DELETE all → INSERT all" 을 폐기하고, 매칭 우선순위(`customer_code` → `rrn_front+rrn_back` 13자리 조합 → `name+phone1`) 로 기존 고객을 찾아 업데이트, 매칭 실패 시만 신규 INSERT. **엑셀 빈 셀 = 기존 값 보존** (덮어쓰지 않음) · **웹 전용 필드(`memo`, `reservationAt`) 는 엑셀에 없어 항상 보존** · **엑셀에 없는 기존 고객은 자동 삭제되지 않음** (삭제는 팝업 개별 버튼으로만). 업로드 프리뷰에 예상 업데이트/신규/기존 유지 건수 + 매칭 내역(고객코드/주민번호/이름+전화 각각 몇 건) 을 표시. | 개발팀 |
...
| 2026-05-06 | 1.9.1 | **보안 코드 리뷰 및 강화**. (1) `proxy.ts` 중앙 인증 가드 추가 — Next.js 16 `proxy` 컨벤션으로 모든 비공개 라우트에 Auth.js 인증 검사 적용 (defense-in-depth). (2) 엑셀 import 자동 생성 계정의 고정 비밀번호(`123456`) → CSPRNG 랜덤 24자리로 변경 — 관리자 비밀번호 리셋 전까지 로그인 불가. (3) 엑셀 export API에 same-origin 검사 + 감사 로그 추가 — PII 복호화 포함 대량 다운로드 기록. | 개발팀 |
