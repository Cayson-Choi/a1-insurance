...
- 최근 보안 보강: import/export/context API rate limit, no-store 보안 헤더, import same-origin 검사, proxy.ts 중앙 인증 가드, export same-origin + 감사 로그
- 최근 운영 보강: 엑셀 담당자ID가 사용자 관리에 없으면 import 실행 시 담당자 계정 자동 생성 (랜덤 비밀번호)
...
- 역할: `agent`
- 최초 비밀번호: CSPRNG 랜덤 24자리 (관리자가 비밀번호 리셋 전까지 로그인 불가)
- 권한: 생성/수정/삭제/export/이미지 저장 모두 `false`
- 감사 로그: `user_create`, `source: excel_import`

## 보안 주의사항

- `proxy.ts`가 중앙 인증 가드 역할을 합니다. 새 라우트 추가 시에도 비인증 접근이 자동 차단되지만, 서버 액션/API에서는 반드시 `requireUser()` 등으로 이중 검사하세요.
- PII 키를 변경하면 기존 암호화 데이터 복호화가 실패합니다.
- `.env.local`, Vercel env pull 결과, 엑셀 원본 파일은 커밋하지 않습니다.
- export 권한이 있는 계정은 개인정보 파일을 내려받을 수 있으므로 운영 계정 권한을 최소화합니다. export 실행은 감사 로그에 기록됩니다.
- 엑셀 import 시 자동 생성된 담당자 계정은 랜덤 비밀번호로 생성됩니다. 관리자가 비밀번호를 리셋하기 전까지 로그인할 수 없습니다.
